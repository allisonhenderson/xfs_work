	...
	uint32_t			diroffset)
{
	...
	name.name = (const char *)&rec;
	name.len = sizeof(rec);
	name.type = ATTR_PARENT;

	return xfs_attr_set_deferred(child, tp, &name, child_name->name,
				     child_name->len);
}
	...
	xfs_dir2_dataptr_t		diroffset)
{
	...
	return xfs_attr_remove_deferred(child, tp, &name);
}
...

{
	...
	namebuf = kmem_zalloc_large(namebuf_size, 0);
	if (!namebuf)
		return -ENOMEM;
	...
	for (i = 0; i < alist->al_count; i++) {
		...
		name.name = (char *)xpnr;
		name.len = sizeof(struct xfs_parent_name_rec);
