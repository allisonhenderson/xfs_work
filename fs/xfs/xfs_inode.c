...
xfs_ilock(
	...
	uint			lock_flags)
{
        my_debug("Enter inode:%llu, lock_flags: %s%s%s%s%s%s (0x%X)", ip->i_ino,
                lock_flags & XFS_IOLOCK_EXCL ? "XFS_IOLOCK_EXCL ":"",
                lock_flags & XFS_IOLOCK_SHARED ? "XFS_IOLOCK_SHARED ":"",
                lock_flags & XFS_ILOCK_EXCL ? "XFS_ILOCK_EXCL ":"",
                lock_flags & XFS_ILOCK_SHARED ? "XFS_ILOCK_SHARED ":"",
                lock_flags & XFS_MMAPLOCK_EXCL ? "XFS_MMAPLOCK_EXCL ":"",
                lock_flags & XFS_MMAPLOCK_SHARED ? "XFS_MMAPLOCK_SHARED ":"",
                lock_flags
        );
